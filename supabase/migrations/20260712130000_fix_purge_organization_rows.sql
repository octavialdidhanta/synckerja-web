-- Supabase hosted Postgres does not allow session_replication_role for SECURITY DEFINER RPC.
-- Replace purge with multi-pass delete (FK-safe without disabling triggers).
...
COMMENT ON FUNCTION public._purge_organization_rows(uuid) IS
  'CMS internal: multi-pass tenant purge without session_replication_role (Supabase compatible).';
